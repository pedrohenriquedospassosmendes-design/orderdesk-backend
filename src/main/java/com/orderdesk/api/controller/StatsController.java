...
public class StatsController {
    ...
    public StatsController(StoreRepository stores, ProductRepository products, CustomerOrderRepository orders, BillingService billingService) {
        this.stores = stores;
    ...
    public ResponseEntity<?> ownerStats(@PathVariable Long ownerId) {
        ...
        if (confirmedSalesCount > 0) {
            averageConfirmedTicket = totalSold.divide(BigDecimal.valueOf(confirmedSalesCount), 2, java.math.RoundingMode.HALF_UP);
        }
    ...
    @GetMapping("/customer")
    public ResponseEntity<?> customerStats(@RequestParam(required = false) Long accountId,
        ...
        if (accountId != null) {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("message", "Conta não informada."));
        }
        ...
        for (CustomerOrder order : customerOrders) {
            ...
            statusCount.put(status, statusCount.getOrDefault(status, 0) + 1);
            if (countsAsSale(status)) {
                totalSpent = totalSpent.add(order.getTotal() == null ? BigDecimal.ZERO : order.getTotal());
            }
        }
